...
$(document).ready(function() {
  ...
  $.ajax({
        ...
        success : function (data) {
        	...
        	$("#id_django_filer").html(txt[4]);
        	$("#id_django_adminplus").html(txt[5]);
        	$("#id_jquery_group").html(txt[6]);
        	$("#id_jquery_admin").html(txt[7]);
        	$("#id_bootstrap_group").html(txt[8]);
        	$("#id_bootstrap_admin").html(txt[9]);
        	...
        	$("#id_git").html(txt[14]);
        	$("#id_pip").html(txt[15]);
        	$("#id_virtualenv").html(txt[16]);

        	var disk_sp = txt[17].split(/\//);
        	$("#id_disk_space").html('<span style="color:#080;">' + disk_sp[0] + '</span> / <span style="color:#f00;">' + disk_sp[1] + '</span>');
        	var mem_sp = txt[18].split(/\//);
        	$("#id_memory").html('<span style="color:#080;">' + mem_sp[0] + '</span> / <span style="color:#f00;">' + mem_sp[1] + '</span>');
        	$("#id_db_backup").html('<span style="color:#00f;">' + txt[19] + '</span>');
        	var cpu = txt[20].split(/\//);
        	$("#id_cpu").html('<span style="color:#f00;">' + cpu[0] + '</span> / <span style="color:#080;">' + cpu[1] + '</span> / <span style="color:#00f;">' + cpu[2] + '</span>');

        	$("#id_news_n").html('<i>' + txt[21] + '</i>');
        	$("#id_news_s").html('<span style="color:#00f;">' + txt[22] + '</span>');
        	$("#id_member_n").html('<i>' + txt[23] + '</i>');
        	$("#id_member_s").html('<span style="color:#00f;">' + txt[24] + '</span>');
        	$("#id_pub_n").html('<i>' + txt[25] + '</i>');
        	$("#id_pub_s").html('<span style="color:#00f;">' + txt[26] + '</span>');
        	$("#id_rot_n").html('<i>' + txt[27] + '</i>');
        	$("#id_rot_s").html('<span style="color:#00f;">' + txt[28] + '</span>');
        	$("#id_spe_n").html('<i>' + txt[29] + '</i>');
        	$("#id_spe_s").html('<span style="color:#00f;">' + txt[30] + '</span>');
    	}
    });
