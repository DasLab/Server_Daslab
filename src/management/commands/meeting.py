...
class Command(BaseCommand):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.types = {'ES': 'EteRNA Special', 'GM': 'Group Meeting', 'JC': 'Journal Club', 'FS': 'Flash Slides'}
        self.msg_handles = []
    ...
    def slack_id_error(sunet_id, name, role):
        if sunet_id == 'none':
            self.stdout.write('\033[41mERROR\033[0m: %s (\033[94m%s\033[0m) not found.' % (role, name))
        elif sunet_id == 'ambiguous':
            self.stdout.write('\033[41mERROR\033[0m: %s (\033[94m%s\033[0m) is ambiguate (more than 1 match).' % (role, name))
        else:
            self.stdout.write('\033[41mERROR\033[0m: %s (\033[94m%s\033[0m) not available in database.' % (role, name))
    ...
    def handle(self, *args, **options):
        ...
        try:
            ...
            target_date = datetime.strptime("%s %s" % (result['this']['date'], year), '%b %d %Y')
            target_date_str = datetime.strftime(target_date, '%b %d %Y')
            ...
            if result['this']['type'] == 'N/A':
                ...
                if result['this']['note']:
                    ...
                    msg_this += '.'
                self.msg_handles.append( (self.send_to('general', channel=True), '', [{"fallback": 'Reminder', "mrkdwn_in": ["text"], "color": "good", "title": 'Group Meeting Reminder', "text": msg_this, "thumb_url": 'https://daslab.stanford.edu/site_media/images/group/logo_bot.jpg'}]) )
                ...
                if meeting_date.date() != target_date.date():
                    ...
                    send_notify_slack(self.send_to(who_id), '', [{"fallback": 'ERROR', "mrkdwn_in": ["text"], "color": "warning", "text": 'Mismatch in Schedule Spreadsheet date. It seems to be not up-to-date.\nFlash Slide has *`NOT`* been setup yet for this week! Please investigate and fix the setup immediately.'}])
                    flag_mismatch = True
                    sys.exit(1)
                ...
                if temp.status_code != 200:
                    ...
                    if IS_SLACK:
                        ...
                        send_notify_slack(self.send_to(who_id), '', [{"fallback": 'ERROR', "mrkdwn_in": ["text"], "color": "warning", "text": 'FlashSlide was created but failed on sharing to the group.\nFlash Slide setup is *`NOT`* complete! Please investigate and fix the setup immediately.'}])
                    ...
                    for name in names:
                        ...
                        if note == 'endofrotationtalk' and BOT['SLACK']['REMINDER']['ROT']['REMINDER_1']:
                            if GROUP.find_type(sunet_id) == 'roton':
                                ...
                                self.msg_handles.append( (self.send_to(who_id), '', [{"fallback": 'Reminder', "mrkdwn_in": ["text"], "color": "good", "text": msg_who}]))
                            else:
                ...
                if note == 'endofrotationtalk' and BOT['SLACK']['REMINDER']['ROT']['REMINDER_ADMIN']:
                    self.msg_handles.append( (send_to, '', [{"fallback": 'REMINDER', "mrkdwn_in": ["text"], "color": "warning", "text": '*REMINDER*: Add *RotationStudent* entry for _%s_.' % reminder_date_str}]) )
                if result['this']['type'] == 'JC' and BOT['SLACK']['REMINDER']['JC']['REMINDER_ADMIN']:
                    self.msg_handles.append( (send_to, '', [{"fallback": 'REMINDER', "mrkdwn_in": ["text"], "color": "warning", "text": '*REMINDER*: Add *JournalClub* entry for _%s_.' % reminder_date_str}]) )
                elif result['this']['type'] == 'ES' and BOT['SLACK']['REMINDER']['ES']['REMINDER_ADMIN']:
                    self.msg_handles.append( (send_to, '', [{"fallback": 'REMINDER', "mrkdwn_in": ["text"], "color": "warning", "text": '*REMINDER*: Add *EternaYoutube* entry for _%s_.' % reminder_date_str}]) )

                send_to = self.send_to('general', channel=True)
                super_prefix = '*Extended/Super* ' if result['this']['type'] == 'FS' else ''
                self.msg_handles.append( (send_to, '', [{"fallback": 'Reminder', "mrkdwn_in": ["text", "fields"], "color": "good", "title": 'Group Meeting Reminder', "text": 'Hi all,\n\nThis is a reminder that group meeting will be %s*`%s`* for this week.\n' % (super_prefix, type_this), "thumb_url": 'https://daslab.stanford.edu/site_media/images/group/logo_bot.jpg', "fields": [{'title': 'Date', 'value': '_%s_' % reminder_date_str, 'short': True}, {'title': 'Time & Place', 'value': '_%s @ %s_' % (result['time']['start'], result['place']), 'short': True}, {'title': 'Type', 'value': '`%s`' % type_this, 'short': True}, {'title': 'Presenter', 'value': '%s' % ', \n'.join(ids), 'short': True}] }]) )
            ...
            if result['last']['note'].lower().replace(' ', '') == 'endofrotationtalk' and BOT['SLACK']['REMINDER']['ROT']['REMINDER_2']:
                ...
                self.msg_handles.append( (self.send_to(who_id), '', [{"fallback": 'REMINDER', "mrkdwn_in": ["text"], "color": "warning", "text": '*REMINDER*: Revoke permissions (_Group Website_ and _Slack Membership_) of recent finished *RotationStudent*.'}]) )
            ...
            if result['next']['type'] == 'N/A':
                ...
                if result['next']['note']:
                    ...
                    msg_next += '.'
                self.msg_handles.append( (self.send_to('general', channel=True), '', [{"fallback": 'Reminder', "mrkdwn_in": ["text"], "color": "439fe0", "text": msg_next}]) )
            else:
                ...
                send_to = self.send_to('general', channel=True)
                self.msg_handles.append( (send_to, '', [{"fallback": 'Reminder', "mrkdwn_in": ["text", "fields"], "color": "439fe0", "text": 'For next week: \n', "thumb_url": 'https: //daslab.stanford.edu/site_media/images/group/logo_bot.jpg', "fields": [{'title': 'Date', 'value': '_%s_' % datetime.strftime(next_date, '%b %d %Y (%a)'), 'short': True}, {'title': 'Time & Place', 'value': '_%s @ %s_' % (result['time']['start'], result['place']), 'short': True}, {'title': 'Type', 'value': '`%s`' % type_next, 'short': True}, {'title': 'Presenter', 'value': '%s' % ', \n'.join(ids), 'short': True}] }]) )
            ...
            if result['this']['type'] != 'N/A':
                year = (today + timedelta(days=offset_1)).date().year
            ...
            if IS_SLACK:
                (who_id, _) = find_slack_id(ppls['weekly']['flash slide']['main'])
                send_notify_slack(self.send_to(who_id), '', [{"fallback": 'ERROR', "mrkdwn_in": ["text"], "color": "warning", "text": 'FlashSlide table in MySQL database, presentation in Google Drive, and posted messages in Slack are rolled back.\nFlash Slide has *`NOT`* been setup yet for this week! Please investigate and fix the setup immediately.'}])
            else:
        ...
        if (not DEBUG):
            (who_id, _) = find_slack_id(ppls['weekly']['flash slide']['main'])
            send_notify_slack(self.send_to(who_id), '', [{"fallback": 'SUCCESS', "mrkdwn_in": ["text"], "color": "good", "text": '*SUCCESS*: Scheduled weekly *Flash Slides Setup* finished @ _%s_. Please also paste the link in the Schedule Spreadsheet.\n' % time.ctime()}])
