...
pip $(cut -d$'\t' -f36 cache/stat_sys.txt | sed 's/ $*//') $(tput sgr 0)" >> ~/.ver_txt
...
echo "$(tput setab 22) apache $(cut -d$'\t' -f7 cache/stat_sys.txt | sed 's/ $*//') | \
curl $(cut -d$'\t' -f28 cache/stat_sys.txt | sed 's/ $*//') | \
gdrive $(cut -d$'\t' -f26 cache/stat_sys.txt | sed 's/ $*//') | \
pandoc $(cut -d$'\t' -f27 cache/stat_sys.txt | sed 's/ $*//') | \
gitinspector $(cut -d$'\t' -f19 cache/stat_sys.txt | sed 's/ $*//') $(tput sgr 0)" >> ~/.ver_txt
...
echo "$(tput setab 20) jquery $(cut -d$'\t' -f12 cache/stat_sys.txt | sed 's/ $*//') | \
...
echo "$(tput setab 55) crontab $(cut -d$'\t' -f4 cache/stat_sys.txt | sed 's/ $*//') | \
...
echo -n "$(tput setab 171) nano $(cut -d$'\t' -f25 cache/stat_sys.txt | sed 's/ $*//') | \
boto $(cut -d$'\t' -f29 cache/stat_sys.txt | sed 's/ $*//') | \
gvizapi $(cut -d$'\t' -f22 cache/stat_sys.txt | sed 's/ $*//') | \
mysql $(cut -d$'\t' -f6 cache/stat_sys.txt | sed 's/ $*//') | \
